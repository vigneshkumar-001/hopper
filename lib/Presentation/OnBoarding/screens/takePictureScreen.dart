      ...
      bottomNavigationBar: CustomBottomNavigation.bottomNavigation(
        onTap: () async {
          ...
          if (_capturedImage != null) {
            await controller.userProfileUpload(
  ...
  Widget _buildCamera() => Center(
    child: Column(
      ...
      children: [
        ...
        const Text('Position yourself within the frame'),
      ],
    ),
  );
}
...
class _TakePictureState extends State<TakePicture> {
  ...
  void _processCameraImage(CameraImage image) async {
    ...
    try {
      ...
      final inputImage = InputImage.fromBytes(
        bytes: image.planes[0].bytes, // Just the first plane for nv21 format
        metadata: InputImageMetadata(
          size: Size(image.width.toDouble(), image.height.toDouble()),
          rotation: InputImageRotation.rotation0deg, // or calculate based on camera
          format: InputImageFormat.nv21,
          bytesPerRow: image.planes[0].bytesPerRow,
        ),
      );
